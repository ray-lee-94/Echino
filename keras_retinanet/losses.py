...
# def _center_loss_func(y_true, y_pred, alpha, num_classes, centers):
#     labels = y_true[:, :, :-1]
#     anchor_state = y_true[:, :, -1]  # -1 for ignore, 0 for background, 1 for object
...
#     normalizer = keras.backend.maximum(1.0, normalizer)
#
#     # assert feature_dim == classification.get_shape()[1]
#     labels = k.reshape(labels, [-1])
#     labels = tf.to_int32(labels)
#     centers_batch = tf.gather(centers, labels)
#
#     diff = (1 - alpha) * (centers_batch -classification)
...
# centers = k.zeros([num_classes, 9])

# @functools.wraps(_center_loss_func)
# def center_loss(y_true, y_pred):
#     return _center_loss_func(y_true, y_pred, alpha, num_classes, centers)
